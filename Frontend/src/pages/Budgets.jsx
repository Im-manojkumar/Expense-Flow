...
export default function Budgets() {
  ...
  const getStyleForStatus = (percentage) => {
    if (percentage >= 100) return { iconColor: 'text-[#C62828]', iconBg: 'bg-[#FFEBEE]', barColor: 'bg-error', borderColor: 'border-[#ffdad6]' };
    if (percentage >= 80) return { iconColor: 'text-[#EF6C00]', iconBg: 'bg-[#FFF3E0]', barColor: 'bg-[#FF9800]', borderColor: 'border-surface-container-low' };
  ...
  const totalRemaining = budgets.reduce((sum, b) => {
    const rem = parseFloat(b.amount) - parseFloat(b.actual_expense);
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-lg">
        {loading ? (
          ...
          budgets.map(budget => {
            ...
            const spent = parseFloat(budget.actual_expense);
            const limit = parseFloat(budget.amount);
            const percentage = Math.min(budget.utilization_percentage, 100);
              ...
              <div key={budget.id} className={`bg-surface-container-lowest rounded-[24px] p-lg shadow-[0px_4px_20px_rgba(0,0,0,0.04)] border ${style.borderColor} flex flex-col gap-md relative overflow-hidden group hover:shadow-[0px_10px_32px_rgba(0,0,0,0.08)] transition-all duration-300`}>
                <div className="flex justify-between items-start">
                  <div className="flex items-center gap-sm">
                    ...
                    <div>
                      <h3 className="font-headline-md text-headline-md text-on-surface">{budget.category || 'All Categories'}</h3>
