...
export default function Expenses() {
  ...
  const fetchExpenses = async () => {
    try {
      ...
      const rawExpenses = expensesRes.data.results || expensesRes.data;
      ...
      const merged = mappedExpenses.sort((a, b) => b.date - a.date);
  ...
  return (
    <div className="space-y-xl pb-xl">
      <div className="mb-xl flex flex-col md:flex-row gap-md justify-between items-start md:items-center">
        <h1 className="font-headline-lg-mobile md:font-headline-lg text-headline-lg-mobile md:text-headline-lg text-on-surface">Expenses</h1>
        <div className="flex w-full md:w-auto gap-sm">
          <div className="relative w-full md:w-64">
            <span className="material-symbols-outlined absolute left-3 top-1/2 -translate-y-1/2 text-outline">search</span>
            <input className="w-full pl-10 pr-4 py-2 bg-surface-container-lowest border border-outline-variant rounded-lg text-body-md focus:outline-none focus:border-primary focus:ring-2 focus:ring-primary/20 transition-all text-on-surface" placeholder="Search expenses..." type="text"/>
          </div>
          <input 
            type="file" 
            accept=".csv" 
      ...
      <div className="bg-surface-container-lowest rounded-xl shadow-[0px_4px_20px_rgba(0,0,0,0.04)] p-lg">
        {loading ? (
          <p className="text-on-surface-variant py-4">Loading expenses...</p>
        ) : Object.keys(groupedTransactions).length === 0 ? (
          <p className="text-on-surface-variant py-4">No expenses found.</p>
        ) : (
